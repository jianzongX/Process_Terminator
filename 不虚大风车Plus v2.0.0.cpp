...
void PerformCleanOperation(HWND hwnd) {
    ...
    // 检查主要进程状态
    bool iClientRunning = IsProcessRunning(L"iClient.exe");
...
void PerformRestoreOperation(HWND hwnd) {
    ...
    const std::wstring exePath = L"C:\\Program Files (x86)\\Cooltion\\Elearning\\iConsoleApp.exe";
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        ...
        // 创建界面元素
        CreateWindowW(L"STATIC", L"不虚大风车Plus v2.0.0",
            WS_VISIBLE | WS_CHILD,
            80, 20, 180, 20,
            hwnd, NULL, NULL, NULL);
